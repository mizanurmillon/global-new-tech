...
@section('content')
    <div class="content-wrapper">
        <x-breadcrumbs title="Services" subtitle="Manage all core services" />

        <div class="card">
            <div class="card-body">
                {{-- <x-table-header title="Services" :route="route('admin.services.create')" /> --}}
                <x-table-header title="Services" />
    ...
    <script>
        $(function() {
            ...
            $('#status_form').on('submit', function(e) {
                ...
                $.ajax({
                    url: '{{ route('admin.services.status', ':id') }}'.replace(':id', id),
                    type: 'PATCH',
                    data: {
                        is_active: isEnabled
                    },
