...
@section('content')
    <div class="content-wrapper">
        <x-breadcrumbs title="Technologies" subtitle="Manage all technologies" />

        <div class="card">
            <div class="card-body">
                <x-table-header title="Technologies" :route="route('admin.technologies.create')" />
    ...
    <script>
        $(function() {
            let dt = $('#data-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.technologies.index') }}",
            ...
            $('#status_form').on('submit', function(e) {
                ...
                $.ajax({
                    url: '{{ route('admin.technologies.status', ':id') }}'.replace(':id', id),
                    type: 'PATCH',
                    data: {
                        is_active: isEnabled
                    },
            ...
            $('#delete_modal_clear').on('submit', function(e) {
                ...
                $.ajax({
                    url: "{{ route('admin.technologies.destroy', ':id') }}".replace(':id', id),
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
